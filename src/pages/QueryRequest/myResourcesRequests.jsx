...
import { getAllUserWithoutLoginIdAction, getAssetRequestByResourceAction } from "../../redux/actions/userAction";
import CustomPagination from "../../components/genericModal/CustomPagination";

export default function MyResourcesRequests() {
    ...
    useEffect(() => {
        let req = {
            search_text: loggedInUser?.userId,
            limit: 10,
            isPagination: true,
            page: 1,
        };
    ...
    useEffect(() => {
        let req = {
            userId: selectedResource?.value ? selectedResource?.value : myResources?.[0]?.userId
        }

        dispatch(getAssetRequestByResourceAction(req))
    }, [myResources, selectedResource])
    useEffect(() => {
        if (myResources) {
            setSelectedResource({
                value: myResources?.[0]?.userId,
                label: myResources?.[0]?.firstName + ' ' + myResources?.[0]?.lastName
            })
        }
    }, [myResources])
    ...
    const MY_RESORCES = myResources?.length > 0
        ? [
                    ...
                    <Row className="mb-3">
                        <Col lg={12}>
                            <div className="d-flex flex-wrap justify-content-between align-items-center ">
                                <h5 className="mb-2">My Resources Query List</h5>
